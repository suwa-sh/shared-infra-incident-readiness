...
複数顧客から SaaS 連携(SNS スケジューラ / ブログ / タスク管理 等)を委託される**マーケ自動化運用者**を題材に、共有インフラ責任モデルの三層へ写像した記入例。「自分が委託先として何を SLA 化すべきか」を 1 度埋めて明文化する worked example。

## When to use this

- 自社が「委託先(運用者)」側で、何を SLA 化・通知すべきか整理したい
- 三層構造(委託元 / 運用者 / SaaS)への写像のしかたを具体例で見たい
...
- 運用者は **委託先** なので、SaaS 側で事故が起きたときに顧客へ何を・いつ通知するかの責任(RB01/RB02/RB11)を負う。
- 鍵・ハッシュ管理(RB06)は運用者が Accountable、実装は SaaS が Responsible という分担になる。
- この表を埋めておくと、SaaS インシデント時に「自分が委託先として何を SLA 化すべきか」(委託元への第一報 SLA = DPA03)が事故前に見える。

`check-responsibility` で PASS するのは、全項目に説明責任の分裂や未割当が無く、明確な単一オーナーが置かれているため(→ 採点ロジックは [01](01_responsibility_boundary.md))。
